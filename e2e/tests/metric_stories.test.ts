...
test.describe('Metric', () => {
  ...
  test.describe('Layout configuration', () => {
    ...
    test(`with default secondary and 'after' secondary label position`, async ({ page }) => {
      await common.expectChartAtUrlToMatchScreenshot(page)(
        `http://localhost:9001/?path=/story/metric-alpha--layout&globals=toggles.showHeader:true;toggles.showChartTitle:false;toggles.showChartDescription:false;toggles.showChartBoundary:false;theme:light&knob-Title=Count of records&knob-Subtitle=Little description of this component&knob-Visualization type=bar&knob-Progress bar direction=vertical&knob-Trend data points=30&knob-Trend shape=area&knob-Trend a11y title=The Cluster CPU Usage trend&knob-Trend a11y description=The trend shows a peak of CPU usage in the last 5 minutes&knob-Progress max=100&knob-Is numeric metric=true&knob-Value=55.23&knob-Value prefix=&knob-Value postfix= %&knob-Show extra_Secondary metric=true&knob-Use custom extra_Secondary metric=&knob-Custom extra_Secondary metric=last <b>5m</b>&knob-Secondary metric value_Secondary metric=87.20&knob-Secondary metric trend icon_Secondary metric=↑&knob-Secondary metric trend icon position_Secondary metric=after&knob-Label_Secondary metric=Last week&knob-Color by value_Secondary metric=true&knob-Secondary metric value color_Secondary metric=rgb(93, 191, 149)&knob-Secondary metric label position_Secondary metric=after&knob-Aria description_Secondary metric=This is a description&knob-Metric color_Colors=rgb(246,	217,	143)&knob-Use value color_Colors=&knob-Value color_Colors=#065B58&knob-Use blending background_Colors=&knob-Blending background_Colors=rgba(255,255,255,1)&knob-Bar background_Colors=rgb(194,	201,	214)	&knob-Show metric icon_Text configuration and position=&knob-Metric icon align_Text configuration and position=right&knob-Show value icon=&knob-Title and subtitle alignment_Text configuration and position=left&knob-Title weight_Text configuration and position=normal&knob-Primary metric position_Text configuration and position=top&knob-Primary metric alignment_Text configuration and position=left&knob-Primary metric font size mode_Text configuration and position=default&knob-Primary metric font size (only if custom font size selected)_Text configuration and position=40&knob-Extra element alignment_Text configuration and position=left`,
        {
          action: async () => await common.setResizeDimensions(page)({ height: 180, width: 180 }),
        },
      );
