...
test.describe('Metric', () => {
  ...
  test(`New layout configuration with default secondary and 'before' badge position`, async ({ page }) => {
    await common.expectChartAtUrlToMatchScreenshot(page)(
      `http://localhost:9001/?path=/story/metric-alpha--new-layout-config&globals=toggles.showHeader:true;toggles.showChartTitle:false;toggles.showChartDescription:false;toggles.showChartBoundary:false;theme:light&knob-Title=Count of records&knob-Subtitle=Little description of this component&knob-Visualization type=bar&knob-Progress bar direction=vertical&knob-Trend data points=30&knob-Trend shape=area&knob-Trend a11y title=The Cluster CPU Usage trend&knob-Trend a11y description=The trend shows a peak of CPU usage in the last 5 minutes&knob-Progress max=100&knob-Is numeric metric=true&knob-Value=55.23&knob-Value prefix=&knob-Value postfix= %&knob-Show extra_Secondary metric=true&knob-Use custom extra_Secondary metric=&knob-Custom extra_Secondary metric=last <b>5m</b>&knob-Secondary metric value_Secondary metric=87.20 ↑&knob-Label_Secondary metric=Last week&knob-Color by value_Secondary metric=true&knob-Secondary metric value color_Secondary metric=#A71627&knob-Secondary metric value position_Secondary metric=before&knob-Metric color_Colors=#61A2FF&knob-Use value color_Colors=&knob-Value color_Colors=#1750BA&knob-Use blending background_Colors=&knob-Blending background_Colors=rgba(255,255,255,1)&knob-Bar background_Colors=&knob-Show metric icon_Text configuration and position=&knob-Metric icon align_Text configuration and position=right&knob-Show value icon=&knob-Title and subtitle alignment_Text configuration and position=left&knob-Title weight_Text configuration and position=regular&knob-Primary metric position_Text configuration and position=top&knob-Primary metric alignment_Text configuration and position=left&knob-Primary metric font size mode_Text configuration and position=default&knob-Primary metric font size (only if custom font size selected)_Text configuration and position=40&knob-Extra element alignment_Text configuration and position=left&knob-Show breakpoint_Text configuration and position=m&knob-Custom size_Text configuration and position=&knob-Metric height_Text configuration and position=400&knob-Metric width_Text configuration and position=400`,
      {
        action: async () => await common.setResizeDimensions(page)({ height: 180, width: 180 }),
      },
    );
