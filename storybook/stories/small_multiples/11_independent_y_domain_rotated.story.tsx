...
export const Example: ChartsStory = (_, { title, description }) => {
  ...
  const rotation = select('Rotation', { '90': 90, '-90': -90 }, 90);
  const seriesType = select('Series type', { Bar: 'bar', Line: 'line' }, 'bar');

  const Series = seriesType === 'bar' ? BarSeries : LineSeries;

  return (
    <Chart title={title} description={description} size={['100%', 500]}>
      <Settings baseTheme={useBaseTheme()} showLegend={false} rotation={rotation} />
