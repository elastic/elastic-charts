...
/**
 * TODO: Handle force pushing to base ref. Currently check will return true.
 */
async function run() {
  ...
  setStatus('pending', `Checking if PR is up-to-date with ${inputs.baseRef}.`);
  ...
  try {
    ...
    if (baseCompare.behind_by === 0) {
      setStatus('success', `PR is up-to-date with ${inputs.baseRef}.`);
      return;
    }
