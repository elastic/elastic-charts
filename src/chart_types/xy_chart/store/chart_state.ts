...
export class ChartStore {
  ...
  onLegendItemOver = action((legendItemKey: string | null) => {
    if (legendItemKey) {
      ...
      if (legendItem && getSeriesIndex(this.deselectedDataSeries, legendItem.seriesIdentifier) > -1) {
        return;
      }
    }
  ...
  toggleSeriesVisibility = action((legendItemKey: string) => {
    ...
    if (legendItem) {
      ...
      const isDeselected = getSeriesIndex(this.deselectedDataSeries, legendItem.seriesIdentifier) > -1;
      this.updateHighlightedLegendItemKey(legendItemKey, isDeselected);
      this.computeChart();
    }
  });
