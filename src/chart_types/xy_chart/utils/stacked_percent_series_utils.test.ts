...
describe('Stacked Series Utils', () => {
  ...
  const WITH_NULL_DATASET = [{ x: 0, y1: 10, g: 'a' }, { x: 0, y1: null, g: 'b' }, { x: 0, y1: 30, g: 'c' }];
  const STANDARD_DATA_SET_WY0 = [{ x: 0, y0: 2, y1: 10, g: 'a' }, { x: 0, y0: 4, y1: 20, g: 'b' }, { x: 0, y0: 6, y1: 70, g: 'c' }];
  const WITH_NULL_DATASET_WY0 = [{ x: 0, y0: 2, y1: 10, g: 'a' }, { x: 0, y1: null, g: 'b' }, { x: 0, y0: 6, y1: 90, mark: null, g: 'c' }];
  ...
  describe('Format stacked dataset', () => {
    ...
    test('format data without nulls on second series', () => {
      ...
      expect(stacked[0].dataSeries.length).toBe(2);
      expect(stacked[0].dataSeries[0].data.length).toBe(4);
      expect(stacked[0].dataSeries[1].data.length).toBe(4);
      expect(stacked[0].dataSeries[0].data[0]).toMatchObject({
        initialY0: null,
        initialY1: 10,
        x: 1,
        y0: 0,
      ...
      expect(stacked[0].dataSeries[0].data[1]).toMatchObject({
        initialY0: null,
        initialY1: 20,
        x: 2,
        y0: 0,
      ...
      expect(stacked[0].dataSeries[0].data[3]).toMatchObject({
        initialY0: null,
        initialY1: 40,
        x: 4,
        y0: 0,
      ...
      expect(stacked[0].dataSeries[1].data[1]).toMatchObject({
        initialY0: null,
        initialY1: null,
        x: 2,
        y0: 1,
      ...
      expect(stacked[0].dataSeries[1].data[2]).toMatchObject({
        initialY0: null,
        initialY1: 30,
        x: 3,
        y0: 0,
