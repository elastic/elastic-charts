...
import { IndexedGeometryMap } from '../state/indexed_map';
...
function getRadiusFn(data: DataSeriesDatum[], lineWidth: number, radiusRatio: number = 50) {
  ...
  // const [min = 0, max] = extent(data, ({ dot }) => dot);
  const { min, max } = data.reduce(
    (acc, { dot }) =>
      dot === null
        ? acc
        : {
            min: Math.min(acc.min, dot),
            max: Math.max(acc.max, dot),
          },
    { min: Infinity, max: -Infinity },
  );
  const radiusStep = (max - min || max * 100) / Math.pow(radiusRatio, 2);
  return function getRadius(dot: number | null): number {
    if (dot === null) {
      return 0;
    }
    const circleRadius = (dot - min) / radiusStep;
...
function renderPoints(
  ...
  const pointGeometries = dataSeries.data.reduce((acc, datum) => {
    ...
    yDatums.forEach((yDatum, index) => {
      ...
      let y;
      // TODO: scale dots to min/max and add ratio
      ...
      // TODO: derive spatial boolean
      indexedGeometryMap.set(pointGeometry, true);
