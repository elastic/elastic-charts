...
export const computeGeometriesSelector = createCachedSelector(
  ...
  (specs, parentDimensions, theme): { arcs?: ArcGeometry[] } => {
    const sunburstSpecs = getSpecsFromStore<SunburstSpec>(specs, ChartTypes.Sunburst, SpecTypes.Series);
    if (sunburstSpecs.length !== 1) {
      return {};
    }
    return render(sunburstSpecs[0], parentDimensions, theme);
  },
)((state) => state.chartId);
