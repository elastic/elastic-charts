...
import { LinkLabelVM, QuadTreeNode } from '../types/ViewModelTypes';
...
export const linkTextLayout = (
  measure: TextMeasure,
  config: Config,
  nodesWithoutRoom: QuadTreeNode[],
  currentY: Distance[],
  anchorRadius: Distance,
  rawTextGetter: Function,
): LinkLabelVM[] => {
  const { linkLabel } = config;
  const maxDepth = nodesWithoutRoom.reduce((p: number, n: QuadTreeNode) => Math.max(p, n.depth), 0);
  ...
  return nodesWithoutRoom
    .filter((n: QuadTreeNode) => n.depth === maxDepth) // only the outermost ring can have links
    .sort((n1: QuadTreeNode, n2: QuadTreeNode) => Math.abs(n2.x0 - n2.x1) - Math.abs(n1.x0 - n1.x1))
    .slice(0, linkLabel.maxCount) // largest linkLabel.MaxCount slices
    .sort((n1: QuadTreeNode, n2: QuadTreeNode) => {
