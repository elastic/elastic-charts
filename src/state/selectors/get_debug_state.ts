...
import {
  computeAxisVisibleTicksSelector,
  AxisVisibleTicks,
} from '../../chart_types/xy_chart/state/selectors/compute_axis_visible_ticks';
import { computeLegendSelector } from '../../chart_types/xy_chart/state/selectors/compute_legend';
import { computeSeriesGeometriesSelector } from '../../chart_types/xy_chart/state/selectors/compute_series_geometries';
import { getAxisSpecsSelector } from '../../chart_types/xy_chart/state/selectors/get_specs';
...
import { DebugState, DebugStateValue } from './debug_state.types';
...
export const getDebugStateSelector = createCachedSelector(
  ...
  ({ geometries }, legend, axes, axesSpecs): string => {
    ...
    const state: DebugState = {
      ...
      bars: getBarState(seriesNameMap, geometries.bars),
    };
...
const getAxes = (ticks: AxisVisibleTicks, axesSpecs: AxisSpec[]): DebugState['axes'] => {
  return axesSpecs.reduce<DebugState['axes']>(
...
const getBarState = (seriesNameMap: Map<string, string>, barGeometries: BarGeometry[]): DebugState['bars'] => {
  const buckets = new Map<string, DebugState['bars'][number]>();
...
const getLineState = (seriesNameMap: Map<string, string>) => ({
  ...
  seriesLineStyle,
  seriesPointStyle,
}: LineGeometry): DebugState['lines'][number] => {
  const name = seriesNameMap.get(key) ?? '';
...
const getAreaState = (seriesNameMap: Map<string, string>) => ({
  ...
  seriesPointStyle,
  seriesAreaLineStyle,
}: AreaGeometry): DebugState['areas'][number] => {
...
function getLegendState(legendItems: LegendItem[]): DebugState['legend'] {
