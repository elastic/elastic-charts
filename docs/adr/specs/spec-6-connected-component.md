...
On mount, run the `normalize` → `resolveActive` → `buildGeometry` pipeline (memoized on spec identity,
so unrelated re-renders don't recompute it) and start a `requestAnimationFrame` loop that eases
`focusDomain` toward a `targetFocusDomain` via `domainTween`, rebuilds geometry each frame, and calls
`renderer.draw`.
...
`kineticFlywheel`; vertical wheel/drag (or a scrollbar) → lane `scrollOffset`. Initial
