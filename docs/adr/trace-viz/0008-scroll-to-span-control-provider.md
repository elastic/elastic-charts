...
Programmatic scroll-to-span reuses the Flame chart's `controlProviderCallback` pattern: the `Trace`
spec accepts `controlProviderCallback?: Partial<TraceControlCallbacks>` and on mount registers its
`scrollToSpan(id)` implementation with the caller. This is an imperative registration, not a prop
command, so re-triggering the same span id works without workarounds.
