...
domain: trace-viz
owners: []
supersedes: [0033-trace-annotation-geometry-and-hit-testing]
---

# ADR 0030 — Trace annotations: composition, geometry, layering, and hit testing

> Consolidates former ADR 0033 (annotation geometry, layering, and the uniform thin-band hit model).

**Implements:** Spec 29 — Trace annotations
**Relates to:** [ADR 0029](./0029-trace-badge-rendering-architecture.md) (per-frame layout → draw → pick pass reused here), [ADR 0004](./0004-raf-render-loop-and-interaction-model.md) (RAF pipeline and interaction model), [ADR 0026](./0026-collapsible-nesting.md) (display hierarchy / memoization), [ADR 0007](./0007-focus-domain-perform-and-fire.md) (`projectionOffset` re-zero parity)
...
Beyond composition, several geometry and hit-testing decisions were genuinely load-bearing and
non-obvious. Spec 29 needs three annotation kinds — time (a point or a range on the x-scale), lane
(one resolved span lane), and hierarchy (the visible root-to-target route of one resolved span) — to
render as compact adornments that never obscure or steal interaction from the spans, badges, and
segments beneath them:
...
### 1. Annotations compose as child specs
...
### 2. A per-frame layout → draw → pick pass, mirroring badges
...
`drawAnnotations` pass runs after `drawBadges`; and `pickAnnotation` reads the same laid-out
`hitRects`. Layout, draw, and pick therefore never drift.

### 3. Layering: annotations draw above spans/badges, clipped to the plot; rails ride the boundary
...
### 4. Uniform thin-band hit model (edges-only for ranges, rail-only for vertical)
...
### 5. Annotation-first interaction precedence, with pointer-down consistency and gesture suspension
...
### 6. Time re-zero via `projectionOffset`, shared with critical-path intervals
...
### 7. Time annotation placement: time-bar marker (default) vs. plot rail
...
Placement only changes draw geometry — the hit model (Section 4) is unchanged in spirit. A `'timebar'`
...
### 8. Annotation resolution memoized separately from the span pipeline
