...
The components within elastic charts are divided in the `packages/elastic-charts/src/chart_types` directory. Currently, bars, lines, and area charts are in the `xy_chart` directory while `partition_chart` contains sunbursts (including pie charts and donut charts) and treemaps.
