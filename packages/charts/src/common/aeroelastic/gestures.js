...
const appleKeyboard = Boolean(
  window && window.navigator && window.navigator.userAgent && /Macintosh|iPhone|iPad/.exec(window.navigator.userAgent),
);
...
const rawCursorPosition = select((action) => (action.type === 'cursorPosition' ? action.payload : null))(primaryUpdate);

const mouseButtonEvent = select((action) => (action.type === 'mouseEvent' ? action.payload : null))(primaryUpdate);
...
/** @internal */
export const cursorPosition = select(({ cursor }, position) => position || cursor)(gestureStatePrev, rawCursorPosition);
...
export const mouseButton = select((next) => {
  ...
  if (event === 'mouseDown') {
    ...
    return event === 'mouseUp' ? { down: false, up: true, uid } : { down: false, up: false, uid: null };
  }
})(mouseButtonEvent);

/** @internal */
export const mouseIsDown = select(({ mouseIsDown }, next) => (next ? next.event === 'mouseDown' : mouseIsDown))(
  gestureStatePrev,
  mouseButtonEvent,
);
...
export const gestureEnd = select(
  (action) =>
    action && (action.type === 'actionEvent' || (action.type === 'mouseEvent' && action.payload.event === 'mouseUp')),
)(primaryUpdate);
...
const mouseButtonState = select(({ mouseButtonState: { buttonState, downX, downY } }, mouseNowDown, { x, y }) => {
...
/** @internal */
export const actionEvent = select((action) => (action.type === 'actionEvent' ? action.payload : null))(primaryUpdate);
