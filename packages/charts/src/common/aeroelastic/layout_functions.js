...
import { arrayToMap, disjunctiveUnion, flatten, identity, mean, not, removeDuplicates } from './functional';
...
function rawGetId(type /*: string*/) /*: string*/ {
  return `${type}-${uuid()}`;
}
...
const shapesAABB = (shapes) =>
  shapes.reduce((prevOuter, shape) => extend(prevOuter, ...shapeAABB(shape, prevOuter)), identityAABB());
...
export const draggingShape = ({ draggedShape, shapes }, hoveredShape, down, mouseDowned) => {
  const dragInProgress =
    down && shapes.reduce((prev, next) => prev || (draggedShape && next.id === draggedShape.id), false);
...
export const getLocalTransformMatrix = (shapes) => (shape) => {
  ...
  return multiply(invert(shapes.find((s) => s.id === shape.parent).transformMatrix), shape.transformMatrix);
};
...
const contentShape = (allShapes) => (shape) =>
  shape.type === 'annotation' ? contentShape(allShapes)(allShapes.find((s) => s.id === shape.parent)) : shape;
...
const rotationManipulation = (config) => ({ shape, directShape, cursorPosition: { x, y }, alterSnapGesture }) => {
  ...
  const relaxed = alterSnapGesture.includes('relax');
...
const rotationAnnotationManipulation = (
  ...
  const shapeIds = directShapes.map(
    (shape) => shape.type === 'annotation' && shape.subtype === config.rotationHandleName && shape.parent,
  );
...
const resizeAnnotationManipulation = (config, transformGestures, directShapes, allShapes, manipulator) => {
  const shapeIds = directShapes.map(
    (shape) => shape.type === 'annotation' && shape.subtype === config.resizeHandleName && shape.parent,
  );
  const shapes = shapeIds.map((id) => id && allShapes.find((shape) => shape.id === id));
  const tuples = flatten(
    shapes.map((shape, i) => transformGestures.map((gesture) => ({ gesture, shape, directShape: directShapes[i] }))),
  );
...
/** @internal */
export const getAlignDistributeTransformIntents = (alignAction, distributeAction, shapes, selectedShapes) => {
  ...
  if (alignAction && children.length > 1) {
    ...
    const childrenIndex = [...new Array(children.length)].map((_, i) => i);
...
const shapeApplyLocalTransforms = (intents) => (shape) => {
  ...
  const sizeIntents = flatten(
    intents
      .map(
        (intent) => intent.sizes && intent.sizes.length && intent.shapes.find((id) => id === shape.id) && intent.sizes,
      )
      .filter(identity),
  );
  ...
  const localTransformMatrix =
    cumulativeTransformIntents.length > 0
  ...
  const sizeVector = mvMultiply2d(
    cumulativeSizeIntents.length > 0 ? multiply2d(baselineSizeMatrix, cumulativeSizeIntentMatrix) : baselineSizeMatrix,
    shape.baseAB ? [...shape.baseAB, 1] : [shape.a, shape.b, 1],
  );

  // Absorb changes if the gesture has ended
  const absorbChanges =
    transformIntents.length === 0 &&
    sizeIntents.length === 0 &&
    cumulativeTransformIntents.length === 0 &&
    cumulativeSizeIntents.length === 0;
            ...
            for (let n = -1; n < 2; n++) {
              ...
              for (let dim = 0; dim < 2; dim++) {
                ...
                if (
                  ...
                  result[key] = {
                    id: counter++,
                    localTransformMatrix: translate(dim ? midPoint : ss, dim ? ss : midPoint, config.atopZ),
                    a: dim ? radius : 0.5,
                    b: dim ? 0.5 : radius,
                    lowPoint,
                    highPoint,
...
function resizeAnnotation(config, shapes, selectedShapes, shape) {
  ...
  const properShape =
    foundShape &&
    (foundShape.subtype === config.resizeHandleName ? shapes.find((s) => shape.parent === s.id) : foundShape);
  if (!foundShape) {
    return [];
  }
...
const translateShapeSnap = (horizontalConstraint, verticalConstraint, draggedElement) => (shape) => {
  ...
  if (constrainedX || constrainedY) {
    ...
    const snapOffset = translateComponent(
      multiply(rotateZ(matrixToAngle(draggedElement.localTransformMatrix)), translate(snapOffsetX, snapOffsetY, 0)),
    );
    return {
      ...shape,
...
const resizeShapeSnap = (
  ...
  if (constrainedX || constrainedY) {
    ...
    const snapTranslateOffset = translateComponent(
      multiply(rotateZ(angle), translate((1 - multiplier) * -snapOffsetX, (1 - multiplier) * snapOffsetY, 0)),
    );
...
const dissolveGroups = (groupsToDissolve, shapes, selectedShapes) => {
  return {
    shapes: shapes
      .filter((s) => groupsToDissolve.every((g) => s.id !== g.id))
      .map((shape) => {
    ...
    selectedShapes: selectedShapes.filter((s) => !groupsToDissolve.some((g) => g.id === s.id)),
  };
};
...
const getLeafs = (descendCondition, allShapes, shapes) =>
  removeDuplicates(
    (s) => s.id,
    flatten(shapes.map((shape) => (descendCondition(shape) ? allShapes.filter((s) => s.parent === shape.id) : shape))),
  );
...
/** @internal */
export const getHoveredShape = (hoveredShapes) => (hoveredShapes.length > 0 ? hoveredShapes[0] : null);
...
const singleSelect = (prev, config, hoveredShapes, metaHeld, uid, _, boxHighlightedShapes) => {
  ...
  const depthIndex =
    config.depthSelect && metaHeld && (hoveredShapes.length === 0 || hoveredShapes[0].type !== 'annotation')
      ? (prev.depthIndex + (down && !prev.down ? 1 : 0)) % hoveredShapes.length
      : 0;
  return {
    shapes: hoveredShapes.length > 0 ? [hoveredShapes[depthIndex]] : [],
    uid,
    depthIndex: hoveredShapes.length > 0 ? depthIndex : 0,
    down,
...
export const getGrouping = (config, shapes, selectedShapes, groupAction, tuple) => {
  const { selectionOutsideGroup, freshSelectedShapes, freshNonSelectedShapes, preexistingAdHocGroups } = tuple;
  ...
  if (preexistingAdHocGroups.length > 0 && selectionOutsideGroup) {
  ...
  if (selectedShapes.length < 2) {
    ...
    const nonGroupGraphConstituent = (s) =>
      s.subtype !== config.adHocGroupName && !parentedSelectedShapes.some((ss) => s.id === ss.id);
    const dissociateFromParentIfAny = (s) =>
      s.parent && s.parent.startsWith(config.groupName) && preexistingAdHocGroups.some((ahg) => ahg.id === s.parent)
        ? { ...s, parent: null }
        : s;
...
export const getSelectionStateFull = (
  ...
  if (mouseButtonUp || uidUnchanged) {
    return {
      ...prev,
      shapes:
        up && prev.boxHighlightedShapes.length > 0
          ? prev.shapes.concat(prev.boxHighlightedShapes).filter((d, i, a) => a.findIndex((dd) => dd.id === d.id) === i)
          : prev.shapes,
      down,
      uid,
      metaHeld,
...
export const getHoverAnnotations = (config, shapes, selectedPrimaryShapeIds, draggedShape) =>
  shapes
    .filter(
      (shape) => shape && shape.type !== 'annotation' && !selectedPrimaryShapeIds.includes(shape.id) && !draggedShape,
    )
...
export const getSnappedShapes = (
  ...
  if (subtype && ![config.resizeHandleName, config.adHocGroupName, config.persistentGroupName].includes(subtype)) {
