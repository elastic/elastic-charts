...
import { dataSource, getEnrichedData } from './data';
...
import { elementSizes, zoomSafePointerX, zoomSafePointerY } from './utils/dom';
import { observe, toCallbackFn } from './utils/generator';
import { clamp, mix, unitClamp } from './utils/math';
import { axisScale, getDesiredTickCount } from './utils/projection';
 ...
 * @internal
 */
export const timeslipRender = (canvas /*: HTMLCanvasElement*/, ctx /*: CanvasRenderingContext2D*/, getData) => {
  ...
  const config = {
    ...
    locale: 'en-US',
    numUnit: 'short',
    ...(initialDarkMode && themeDark),
    ...(!initialDarkMode && themeLight),
  ...
  const horizontalCartesianAreaPad = [0.04, 0.04];
  const verticalCartesianAreaPad = [0.12, 0.12];
  ...
  function doCartesian(
    ...
    // const panTweenOngoing = interactionState;
  ...
  function renderChartWithTime(
    ...
    // chart title
    renderChartTitle(ctx, config, chartWidth, cartesianTop, aggregationFunctionName);
  ...
  function chartWithTime(ctx, config, interactionState, deltaT) {
    ...
    if (
      ...
      processAction({
        ...
        target: dataSource,
        type: 'dataArrived',
        dataResponse: getEnrichedData(getData(dataDemand)),
      });
  ...
  const setDomElements = () => {
    const chartSizeInfo = elementSizes(canvas, horizontalCartesianAreaPad, verticalCartesianAreaPad);
    interactionState.screenDimensions = chartSizeInfo;
    const { r, g, b } = config.backgroundColor;
    const backgroundColorCSS = `rgb(${r},${g},${b})`;
  ...
  const getPanDeltaPerDragPixel = () => 1 / ((zoomMultiplier() - 1) * interactionState.screenDimensions.innerWidth);
  ...
  const inCartesianBand = (e) => {
    ...
    const { innerTop: cartesianTop, innerBottom: cartesianBottom } = interactionState.screenDimensions;
  ...
  const zoom = (pointerUnitLocation, newZoom, panDelta = 0) => {
    ...
    interactionState.pan =
      unitClamp(
        mix(pointerUnitLocation + panDelta, interactionState.pan, oldInvisibleFraction / newInvisibleFraction),
      ) || 0;
  };
  ...
  const eventHandlersForData = { dataArrived };

  const eventHandlers = new Map([
    [window, eventHandlersForWindow],
    [canvas, eventHandlersForCanvas],
    [dataSource, eventHandlersForData],
  ]);

  function* handleEvents() {
    for (;;) {
      const e = yield;
      const handler = eventHandlers.get(e.target)[e.type];
