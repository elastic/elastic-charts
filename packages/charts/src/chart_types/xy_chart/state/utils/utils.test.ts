...
import { HistogramModeAlignments } from '../../utils/specs';
...
describe('Chart State utils', () => {
  it('should compute and format specifications for non stacked chart', () => {
    ...
    const domains = computeSeriesDomains([spec1, spec2], scaleConfig, [], { locale: 'en-US' });
  ...
  describe('Geometries counts', () => {
    ...
    test('can compute area geometries with custom style', () => {
      ...
      expect(geometries.geometries.areas[0]?.value.style.area).toEqual({
        visible: true,
        fill: 'area-fill-custom-color',
        opacity: 0.2,
      });
