...
describe('computeChartLayoutSelector', () => {
  describe('with deterministic text measurement', () => {
    ...
    test('multi-iteration loop converges to a self-consistent (fixed-point) layout', () => {
      // X axis with long ordinal labels creates a real feedback loop.
      // The bootstrap measurement (taken against a unit-range scale) can't predict this,
      // so `computeChartLayout` must iterate to find a self-consistent layout.
      ...
      MockStore.addSpecs(
        [
          ...
          ySpec,
          xSpec,
          MockSeriesSpec.bar({ data: longCategoryData, xScaleType: ScaleType.Ordinal }),
        ],
        store,
      );
