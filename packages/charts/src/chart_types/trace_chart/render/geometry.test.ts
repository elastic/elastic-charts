...
describe('buildGeometry', () => {
  const spans = [span('b', 500, 800), span('a', 100, 400), span('c', 200, 300)];

  it('sorts spans ascending by start', () => {
    const geom = buildGeometry(spans, canvasSize, focusDomain, 0, style, 'linear');
  ...
  it('does not mutate the input spans array', () => {
    ...
    buildGeometry(spans, canvasSize, focusDomain, 0, style, 'linear');
  ...
  it('lane y = index * laneHeight - scrollOffset', () => {
    ...
    const geom = buildGeometry(spans, canvasSize, focusDomain, scrollOffset, style, 'linear');
  ...
  it('domain reflects the full extent of all spans', () => {
    const geom = buildGeometry(spans, canvasSize, focusDomain, 0, style, 'linear');
  ...
  it('focusDomain is passed through unchanged', () => {
    ...
    const geom = buildGeometry(spans, canvasSize, custom, 0, style, 'linear');
    expect(geom.focusDomain).toEqual(custom);
  });

  it('xScaleType is threaded through to the geometry', () => {
    const geomTime = buildGeometry(spans, canvasSize, focusDomain, 0, style, 'time');
    const geomLinear = buildGeometry(spans, canvasSize, focusDomain, 0, style, 'linear');
  ...
  it('empty spans produce a zero domain and a valid (no-throw) scale', () => {
    const geom = buildGeometry([], canvasSize, focusDomain, 0, style, 'linear');
  ...
  it('zero-width focusDomain guard → scale always returns plot.left without throwing', () => {
    ...
    const geom = buildGeometry(spans, canvasSize, zeroFocus, 0, style, 'linear');
