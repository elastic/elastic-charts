...
import { computeMaxScroll, computeZoomMax, domainToZoomPan, hasViewKeyChanged, resolveMinVisibleExtent } from '../render/interaction';
 ...
 */
export function maybeEmitDiagnostics(c: TraceCanvasController, spec: TraceSpec, report: TraceDataDiagnostics, key: string) {
...
export function runFrame(c: TraceCanvasController, deltaT: number) {
  ...
  const props = c.deps.getProps();
  const { traceSpec, chartDimensions: { width, height } } = props;
  if (!traceSpec) return;

  const { spans: pipelineSpans, depthBySpan, hasParents, maxDepth, domain, projectionOffset, emptyReason, criticalIntervals, diagnostics: diagnosticsReport, diagnosticsKey } = getPipeline(c, traceSpec);
  ...
  const { spans, disclosure: disclosureByLane, rolledUpCriticalIntervals } = getCollapseOutput(c, pipelineSpans, effectiveCollapsed, depthBySpan, criticalIntervals);
  ...
  if (traceSpec.badgeAccessor) {
    ...
    const badgesByLane = layoutBadges(geomBase, style, badgeSize, c.measureBadgeText, c.measureLabelText, firstLane, lastLane);
