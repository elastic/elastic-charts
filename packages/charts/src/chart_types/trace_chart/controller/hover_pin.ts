...
import { buildTraceAnnotationEvent, buildTraceBadgeEvent, buildTraceEvent, buildTraceTooltipInfo } from '../render/tooltip';
...
export function rebuildTooltip(c: TraceCanvasController, span: NormalizedSpan, index: number, domainMin: number, region: HoverRegion, segmentIndex: number) {
  ...
  c.hover.tooltipInfo = buildTraceTooltipInfo(span, index, domainMin, region, span.color ?? style.activeSegmentColor, segmentIndex, criticalIntervals, displayParentName);
}
 ...
 */
export function enterBadgeHover(c: TraceCanvasController, hit: BadgePickResult, span: NormalizedSpan, x: number, y: number): void {
