...
function span(id: string, start: number, end: number, opts: { parentId?: string; active?: { start: number; end: number }[] } = {}): NormalizedSpan {
  return {
    id,
    name: id,
...
describe('resolveActive', () => {
  it('leaf with no children → one full-duration active segment', () => {
    ...
    expect(result[0]!.active).toEqual([{ start: 100, end: 200 }]);
  });
  ...
  it('one child in the middle → two segments (before and after child)', () => {
    ...
    expect(rootResult.active).toEqual([
      { start: 0, end: 300 },
      { start: 700, end: 1000 },
    ]);
  });
  ...
  it('child at the very start → one trailing segment', () => {
    ...
    const result = resolveActive([root, child]);
    expect(result.find((s) => s.id === 'root')!.active).toEqual([{ start: 400, end: 1000 }]);
  });
  ...
  it('child at the very end → one leading segment', () => {
    ...
    const result = resolveActive([root, child]);
    expect(result.find((s) => s.id === 'root')!.active).toEqual([{ start: 0, end: 600 }]);
  });
  ...
  it('child spans the full parent → empty active (no self time)', () => {
    ...
    const result = resolveActive([root, child]);
    expect(result.find((s) => s.id === 'root')!.active).toEqual([]);
  });
  ...
  it('child that overruns parent end → clamped, no out-of-bounds active segment', () => {
    ...
    expect(result.find((s) => s.id === 'root')!.active).toEqual([{ start: 0, end: 200 }]);
  });
  ...
  it('child that overruns parent start → clamped, no negative-width active segment', () => {
    ...
    expect(result.find((s) => s.id === 'root')!.active).toEqual([{ start: 600, end: 800 }]);
  });
  ...
  it('child entirely outside parent → treated as no children (clock skew / bad data)', () => {
    ...
    const result = resolveActive([root, child]);
    expect(result.find((s) => s.id === 'root')!.active).toEqual([{ start: 0, end: 100 }]);
  });

  it('span with explicit non-empty active passes through unchanged', () => {
    const explicit = span('a', 0, 1000, { active: [{ start: 100, end: 200 }] });
    const child = span('b', 400, 600, { parentId: 'a' }); // would change active if derived
    const result = resolveActive([explicit, child]);
    // active must be the original object (not recomputed)
    expect(result.find((s) => s.id === 'a')!.active).toEqual([{ start: 100, end: 200 }]);
  });

  it('zero-duration span → degenerate leaf produces empty active (start === end)', () => {
  ...
  it('does not mutate input spans', () => {
    ...
    const inputActive = root.active;
    resolveActive([root, child]);
    expect(root.active).toBe(inputActive); // same reference — not touched
  });
